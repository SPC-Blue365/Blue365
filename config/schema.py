...
# 데이터파일 (로컬 전용)
DATA_FILE = "data_v2.xlsx"   # v2(2026-08-05): 49Q 에 C/R·비고 컬럼 추가
...
COL_CRUSHER = "C/R(생산방법)"    # v2 실제 컬럼명
COL_MINE_NOTE = "비고"           # v2 실제 컬럼명 — 수항·만실·구역시각 등 운전 기록

# C/R 값 중 생산이 없는 것들 (전부 이송물량 결측으로 확인됨)
CRUSHER_IDLE = {"운휴", "0", ""}

# === 수항 (Surge Bin / 사일로) ⭐️ (2026-08-05 사용자 확정) ===============
# 광산 채굴분이 OSP 로 가는 경로는 둘이다:
#
#     광산 ─┬─→ [수항] ──→ G/C ──┐
#           └────────→ H/C ──────┴─→ (OSP행 벨트 1개) ─→ OSP
#
#   · 수항 = 사일로 개념. 저장 용량 **10,000톤**, 적재 속도 **약 1,500 t/h**.
#   · 동시 가동(G/C+H/C)이라도 **OSP 로 가는 벨트는 1개**라 거기서 만나 적치된다.
#     → 능력이 가산되지 않는 이유가 이것이다(§CRUSHER_CAPACITY_TPH 의 물리적 근거).
#   · ⭐️ **수항은 버퍼이므로 채굴 품위와 OSP 적재 품위 사이에 지연·혼합이 생긴다.**
#     상류(광산)–하류(야드) 상관이 0.21~0.25 로 낮은 것의 물리적 설명 후보다.
#
# 비고에 남는 수항 기록:
#   "(수항채움 43.11, 3.39, 2400톤)"  → 수항에 넣은 (CaO, MgO, 톤)
#   "수항채움 ( 덤프기준 3,040톤 )"    → 톤만
#   "수항재고부족 종료" · "수항 단독 생산" · "OSP 만실" → 운전 이벤트
# ⚠️ 수항채움 물량은 **그 시점의 OSP 적재가 아니다**(나중에 G/C 를 거쳐 나간다).
SURGE_BIN_CAPACITY_TON = 10_000
SURGE_BIN_FILL_TPH = 1_500
