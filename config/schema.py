...
#: OSP 인출 시트의 `공정구분` — 현장이 **목적지 라인**을 적는 칸이다(§6-0-21).
#: 기존 시트에서 이 값이 '신설' 이면 "기존 OSP 에서 뽑아 신설 라인으로 보냈다" 는 뜻이다.
COL_OSP_PROCESS = "공정구분"

#: OSP 인출 시트의 비고 — 교차인출·라인 분할 표기가 들어온다(§6-0-19)
COL_OSP_NOTE = "비고"           # v2 실제 컬럼명 — 수항·만실·구역시각 등 운전 기록
...
# === OSP 적치장 (2026-08-14 현장 확인) ===
#: 라인별 만실 용량. 실사 재고 최대치가 기존 45,000 · 신설 43,000 으로 한 번도 넘지 않아
#: 독립적으로 뒷받침된다. 배합 최적화의 재고 상한 제약으로 쓴다.
OSP_CAPACITY_TON = 47_000

#: 관측 시작 시점의 라인별 재고 (현장 제공). 실사 기록 6/10 마지막 값과 정확히 일치했다.
OSP_OPENING_STOCK = {"date": "2026-06-10", LINE_OLD: 36_000, LINE_NEW: 30_000}

#: ⚠️ **구역별 재고는 측정하지 않는다**(현장 확인). 잔량은 추정에 의존한다.
#: 따라서 '구역별 기초 재고·품위'로 초기 미매칭분을 메우는 방법은 원리적으로 불가능하다.
OSP_ZONE_STOCK_MEASURED = False

#: 구역은 물리적 '칸'이며 **칸 높이를 넘겨 쌓으면 옆 칸으로 흘러든다**(현장 확인).
#: → 구역 라벨은 엄밀한 경계가 아니라 **근사**다. 구역 단위 품위 매칭의 정밀도 상한이 여기서 생긴다.
OSP_ZONE_SPILLOVER = True

#: 구역 90 이상은 대부분 47Q 물량이다(현장: 80%+). 데이터로도 47Q 적재의 73.4%가 90번 이상인 반면
#: 49Q 는 1.0%뿐이다. 인출 후에는 90번 이전 위치를 만실까지 채운다.
OSP_47Q_ZONES = (90.0, 95.0, 100.0)

# === 야드 분석기 유효 범위 ⭐️ (2026-09-17) ===
#: 석회석의 CaO 는 이 값 아래로 내려갈 수 없다. 그보다 낮은 측정치는 **품위가 아니라
#: 분석기가 헛돈 값**이다 — 실제로 벨트 적재량이 적을 때 몰려 나온다
#: (헛값 구간의 적재량 중앙값 15 vs 정상 35).
#: 물량 정보(load)는 살리고 **품위만 결측 처리**한다. 섞어 두면 품질 지표가 크게 왜곡된다
#: (신설 야드 편차 3.50 → 1.70, §6-0-20).
YARD_CAO_MIN = 30.0
