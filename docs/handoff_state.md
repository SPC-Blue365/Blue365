...
## ⭐️ 현재 상태 (2026-07-23 야간) — 라인 분리로 신호 확인, 그러나 천장 존재
도메인 정정 반영: **기존/신설 = 서로 다른 야드(4-5K/6-7K 킬른)**, **P/W=인출지점**, 품위는 시간인지 매칭.
- 라인 분리 + 시간인지로 상관 **0.17 → 0.34(기존→CNA) / 0.39(신설→45Q)**. 물리적으로 타당.
- 그러나 **설명력 천장 R²≈0.11~0.13** — 상류가 야드 변동의 11~13%만 설명 → 목표(±0.5) 제어엔 부족.
- 원인: 야드시간의 25%만 인출기록(버퍼/직송 미확인), 야드 자기상관 큼(1h≈0.73).
- **다음 결정 필요(사용자)**: ⓐ OSP↔야드 버퍼/직송·추가 투입원·지점실측품위 데이터 확보로 신호 강화, 또는 ⓑ 하이브리드(지속성+상류) 예측 + 방향성 배합추천으로 스코프 현실화. → 아래 §3 참조.
