...
## ⭐️ 현재 최대 이슈 (2026-07-23 저녁)
매칭·ML 파이프라인은 **정상 작동**하나, **예측 신호가 없음**(정직 보고):
- OSP 예상CaO ↔ 야드 CaO 상관 0.17 (시간인지 매칭해도 0.18, 일단위 0.14)
- 베이스라인 시계열CV: 모든 모델이 **평균 예측보다 나쁨**(R²<0, MAE 1.5~2.2 ≫ 목표 0.5)
- → 코드/모델 문제 아님. **데이터 연결(도메인) 문제.** 구역코드 의미·버퍼/혼합·타 피드 확인 필요 → 사용자 질문 대기.
